...
@app.cls(image=whisper_image, gpu="A10G", timeout=120, scaledown_window=60)
class WhisperModel:
    """
    Runs OpenAI Whisper on an A10G GPU.
    ...
    async def transcribe(self, request: dict):
        ...
        try:
            ...
            # Write to temp file with correct extension so ffmpeg can detect the codec
            with tempfile.NamedTemporaryFile(suffix=f".{fmt}", delete=False) as tmp:
                tmp.write(audio_bytes)
                tmp_path = tmp.name

            # Whisper's load_audio uses ffmpeg → handles webm, mp3, wav, etc.
...
# ---------------------------------------------------------------------------
# Endpoint 4: Analyze Technical Interview (CPU — LLM proxy)
# ---------------------------------------------------------------------------

TECH_SYSTEM = """You are an expert technical interview advisor. Given a job description, determine whether the role involves a traditional algorithmic/LeetCode-style coding interview.
...
TECH_USER = """Here is the job description:
...
@app.function(image=llm_image, secrets=[LLM_SECRET], timeout=120)
@modal.fastapi_endpoint(method="POST", label="analyze-technical")
def analyze_technical(request: AnalyzeTechnicalRequest) -> dict:
    user = TECH_USER.format(job_description=request.jobDescription)
    data = call_llm_json(TECH_SYSTEM, user, 0.3, MAX_TOKENS_GENERATE)
    return data


# ---------------------------------------------------------------------------
# Endpoint 5: Health check (simple GET, useful for testing)
# ---------------------------------------------------------------------------
